...
export interface IAddAccountFormData {
  name: string;
...
const CURRENT_FORM_ADD_ACCOUNT: IAddAccountFormData = {
  name: "",
  account_type_id: "1",
  account_number: "",
  balance: "0",
...
export default function AddAccountModal({
  ...
  const {
    ...
    formState: { errors },
    reset,
  } = useForm<IAddAccountFormData>({
    resolver: zodResolver(addAccountFormSchema),
    defaultValues: CURRENT_FORM_ADD_ACCOUNT,
    mode: "onChange",
  });

  const handleFormSubmit = async (data: IAddAccountFormData) => {
    await onSubmit({
      name: data.name,
  ...
  return (
    ...
    >
      <form onSubmit={handleSubmit(handleFormSubmit)} className="p-6 space-y-6">
        ...
        {/* Account Type */}
        <FormFieldSelect<IAddAccountFormData>
          label="Account Type"
